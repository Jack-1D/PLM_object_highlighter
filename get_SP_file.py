...
def alive_get_SP_file():
    ...
    while True:
            ...
            for k in df.keys():
                for sheet in screwSheet:
                    if re.search(sheet, k) != None:
                        combine = pd.concat(
                            [combine, df[k]["TPE Part No."]], axis=0, ignore_index=True
                        )
                        combine = pd.concat(
                            [combine, df[k]["SH Part No."]], axis=0, ignore_index=True
                        )
            combine.name = "料號"
        ...
        try:
            ...
            for k in df.keys():
                if re.search("六角螺柱", k) != None:
                    ...
                    break
            combine = pd.concat(
                [combine, df["TPE Part No."]], axis=0, ignore_index=True
            )
        ...
        try:
            ...
            combine.to_excel(
                f"data_sheets/ME pcb smt nut common parts.xlsx", index=False
            )
        except Exception as e:
            print(f"Error: {e}\n")
            # sp2_logger.info(f"Error: {e}\n")
        ...
        try:
            ...
            ctx = ClientContext(url).with_credentials(user_credentials)
            file_url = "/sites/TPDC%20Web%20Portal/PEC/MED/Shared%20Documents/MED%20Public/MED%20Carton,%20Box%20Dimension%20Search.xlsx"
            response = File.open_binary(ctx, file_url)
            bytes_file_obj = io.BytesIO()
            bytes_file_obj.write(response.content)
            bytes_file_obj.seek(0)
            ...
            for k in df.keys():
                for sheet in carton_pizzabox_sheet:
                    if re.search(sheet, k) != None:
                        if "台北料號" in df[k]:
                            combine = pd.concat(
                                [combine, df[k]["台北料號"]], axis=0, ignore_index=True
                            )
                            combine = pd.concat(
                                [combine, df[k]["上海料號"]], axis=0, ignore_index=True
                            )
                        else:
                            combine = pd.concat(
                                [combine, df[k]["料號"]], axis=0, ignore_index=True
                            )
                            combine = pd.concat(
                                [combine, df[k]["皆為兩地common part"]],
                                axis=0,
                                ignore_index=True,
                            )
            combine.name = "料號"
